    ...
    if cmd == 'start' or cmd == 'начать':
        while True:
            ...
            if COMMAND in ["sqr", "sin", "cos", "tan", "ln", "lg"]:
                ...
                if n1.isdigit() or (n1[0] == '-' and n1[1:].isdigit()) or (n1.count('.') == 1
                                                                           and n1.replace('.', '').isdigit()) or (n1[0] == '-' and n1[1:].replace('.', '').isdigit() and n1.count('.') == 1):
                    NUM_1 = float(n1)
                else:
                ...
                if n2.isdigit() or (n2[0] == '-' and n2[1:].isdigit()) or (n2.count('.') == 1
                                                                           and n2.replace('.', '').isdigit()) or (n2[0] == '-' and n2[1:].replace('.', '').isdigit() and n2.count('.') == 1):
                    NUM_2 = float(n2)
                else:
                    print("Нужно было ввести число!!")
                    continue
